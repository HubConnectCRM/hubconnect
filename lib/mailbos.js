...
// Same endpoint HubConnect iOS already calls (MailBosService.swift
// researchCompany) — MailBos does the real AI research and its own
// cross-account cache/credit accounting; we just consume the result.
export async function researchCompanyMailbos(apiKey, { company, website, email } = {}) {
  const body = {};
  if (company) body.company = company;
  if (website) body.website = website;
  if (email) body.email = email;
  const payload = await mailbosRequest(apiKey, "/api/ext/v1/company-cache", { method: "POST", body });
  return payload.item || {};
}
